...
public class IUCancelar extends javax.swing.JInternalFrame
{
    ...
    public IUCancelar(String l)
    {
        id = l;
        
        select = "SELECT av.idArticuloVendido as ID, av.Nombre, av.Cantidad, av.Precio, av.Subtotal " +
        ...
        try
        {
            ...
            {
                JOptionPane.showInternalMessageDialog(IUMain.desktop,"La lista no existe", "Error en la lista", 2);
                exist = false;
            }
        }
        catch (SQLException ex)
        {
            JOptionPane.showInternalMessageDialog(IUMain.desktop, ex.getMessage(), "Error en la lista", 2);
    ...
    private void initComponents()
    {
        ...
        setFrameIcon(new javax.swing.ImageIcon(getClass().getResource("/Imagenes/addarticulo2.png"))); // NOI18N
        ...
        jButton5.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Imagenes/canceldocument1.png"))); // NOI18N
    ...
    {//GEN-HEADEREND:event_jButton5ActionPerformed
        ...
        if (jTable1.getRowCount() != 0)
        {
            IUMain.control.cancelarLista(id);
            refresh(filaSeleccionada);
            IUMain.articulos.showTotal();
        }
        else
            JOptionPane.showInternalMessageDialog(IUMain.desktop, "No hay lista que cancelar", "Error", 2);
    ...
    {
        jLabel1.setText("Total= $ " + IUMain.control.getTotalFactura(id) + "  ");
    }
    public CellEditorListener Cell()
    {
        CellEditorListener celda = new CellEditorListener() 
        {
            ...
            public void editingStopped(ChangeEvent e) 
            {
                ...
                IUMain.control.updateItemCancelacion(item, id, 2);
                refresh(filaSeleccionada);
                IUMain.articulos.showTotal();
            }
    ...
    {
        item = getItem(filaSeleccionada);
        IUMain.control.updateItemCancelacion(item, id, 1);
        refresh(filaSeleccionada);
        IUMain.articulos.showTotal();
    }
    ...
    {        
        item = getItem(filaSeleccionada);
        IUMain.control.updateItemCancelacion(item, id, 0);
        refresh(filaSeleccionada);
        IUMain.articulos.showTotal();
    }
    ...
    {
        item = getItem(filaSeleccionada);
        IUMain.control.deleteItemCancelacion(item, id);
        filaSeleccionada = -1;
        refresh(filaSeleccionada);
        IUMain.articulos.showTotal();
    }
